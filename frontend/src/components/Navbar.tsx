...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-2 sm:px-4 lg:px-6 pt-2 sm:pt-3 transition-all duration-300 pointer-events-none">
      <nav
        className={`max-w-7xl mx-auto rounded-3xl transition-all duration-300 pointer-events-auto px-3 sm:px-5 py-2 sm:py-2.5 flex items-center justify-between gap-2 lg:gap-3 w-full overflow-hidden ${
          scrolled
      ...
      >
        ...
        <button
          ...
          className="flex items-center gap-2 sm:gap-2.5 group shrink-0 text-left cursor-pointer select-none"
        >
          <div className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center transform group-hover:scale-105 transition-all shrink-0">
            <img
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1 sm:gap-1.5">
              <span className="text-sm sm:text-base xl:text-lg font-black tracking-tight text-slate-900 leading-none">
                โต๊ะจีน<span className="text-red-700 ml-0.5 font-black">รพีพัฒน์</span>
              </span>
              <span className="px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[9px] font-black uppercase tracking-wider bg-gradient-to-r from-amber-600 via-amber-500 to-amber-700 text-white rounded-full shadow-xs border border-amber-300">
                PREMIUM
              </span>
            </div>
            <span className="text-[10.5px] sm:text-xs text-amber-900 font-bold tracking-tight hidden xs:flex items-center gap-1 mt-0.5">
        ...
        {/* ========================================================================= */}
        <div className="hidden lg:flex items-center gap-1 bg-amber-50/80 p-1 rounded-2xl border border-amber-300/70 shadow-2xs shrink">
          {navPages.map((page) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 xl:px-3.5 py-1.5 rounded-xl text-xs xl:text-sm font-bold flex items-center gap-1.5 transition-all whitespace-nowrap cursor-pointer select-none ${
                  isActive
        ...
        <div className="hidden md:flex items-center gap-1.5 lg:gap-2.5 shrink-0">
          ...
          <button
            ...
            className="relative px-2.5 lg:px-3 py-1.5 sm:py-2 rounded-2xl bg-white hover:bg-amber-50 border border-amber-300 text-slate-800 hover:text-red-700 transition-all flex items-center gap-1.5 text-xs xl:text-sm font-bold shadow-2xs whitespace-nowrap cursor-pointer"
            title="ดูประวัติใบเสนอราคา"
          >
            <FileText className="w-3.5 h-3.5 text-red-600" />
            <span className="hidden xl:inline">ประวัติเอกสาร</span>
            {historyCount > 0 && (
              <span className="bg-gradient-to-r from-red-600 to-red-700 text-white text-[9px] w-4 h-4 rounded-full font-bold flex items-center justify-center leading-none shadow-xs">
                {historyCount}
              </span>
            )}
          </button>
          ...
          <a
            href="tel:0813311646"
            className="px-3 xl:px-3.5 py-1.5 sm:py-2 rounded-2xl bg-slate-900 hover:bg-black text-amber-300 font-bold text-xs xl:text-sm flex items-center gap-1.5 border border-amber-500/40 shadow-xs transition-all whitespace-nowrap"
            title="โทรด่วน โต๊ะจีน รพีพัฒน์"
          >
            <Phone className="w-3.5 h-3.5 animate-bounce text-amber-400" />
            <span className="text-white font-extrabold hidden sm:inline">081-331-1646</span>
          </a>
          ...
          <button
            ...
            className={`px-3.5 xl:px-4.5 py-1.5 sm:py-2 rounded-2xl font-black text-xs xl:text-sm flex items-center gap-1.5 shadow-red-glow border-2 transition-all transform hover:scale-103 active:scale-95 whitespace-nowrap cursor-pointer ${
          ...
          >
            <Sparkles className={`w-3.5 h-3.5 ${currentView === 'quotation' ? 'text-red-700 animate-spin' : 'text-amber-300'}`} />
            <span>คำนวณราคา</span>
          </button>
        </div>
